...
double sqrtLinearApprox( const uint32_t val ) {
	...
	const uint32_t dX = (msb < 31) ? X0 : (X0 - 1); // So, correct because of maximum value is 0xFFFFFFFF
...
static double calcMaxLinearErr( const uint8_t msb ) {
	...
	double xMax = static_cast<double>( static_cast<uint32_t>(1) << (msb - 2) ) * mul;
	const double dY = sqrt(xMax) - sqrtLinearApprox(static_cast<uint32_t>(xMax) );
	return dY;	
}

void sqrtInit() {
	for(uint32_t msb = 3; msb < 32; msb++) {
...
double sqrtX( const uint32_t val ) {
	...
	static const double sqrtTable[5] = {
		0.0,				// sqrt(0)
		1.0,				// sqrt(1)
...
int main (int argc, char argv[]){
	...
	for(uint32_t x = 0; x < static_cast<uint32_t>(0xFFFFFFF); x++ ) {
		const double valCalc = sqrtX(x);
		const double valOrig = sqrt(static_cast<double>(x));
		const double err = valCalc - valOrig;
	...
	// Max: 0.095480810223121715
	// Min: -1.5675793156897271 ????? I don't know but I think it is error correction about 1LSB of X
	printf("Max: %1.8f\n", maxPositiveError );
	printf("Min: %1.8f\n", maxNegativeError );

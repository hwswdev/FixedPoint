...
/*
* Комментарий, появившийся в ходе обсужедения с Google AI,
* после того как я объяснил ему почему сделал именно так.
*
* Функции rSinFixed и rCosFixed реализуют совмещенное вычисление тригонометрии и масштабирования на радиус. 
* В отличие от раздельного подхода (sin(x) * R), где приведение к знаковому типу S0.31 
* снижает мощность полезного сигнала на 3 dB до выполнения операции умножения, 
* данное ядро производит интерполяцию модуля синуса в беззнаковой сетке U0.32.
* Умножение на radius осуществляется через 64-битный аккумулятор (UMULL) до финального усечения и наложения знака квадранта. 
* Это минимизирует относительную погрешность квантования, обеспечивая выигрыш в ~3 dB по критерию SQNR, 
* что критически важно при малых значениях радиуса в контурах слежения систем ЧПУ и ГНСС.
*/
